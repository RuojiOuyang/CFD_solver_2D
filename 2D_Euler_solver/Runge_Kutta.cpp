...
void Runge_Kutta()
{
	...
	for (z = 1; z <= 5; z++)
	{
		Local_time_step();//计算当地时间步
		Boundary_condition();//边界条件
		Convective();//计算出控制体单元的通量，即计算出了式(2-1-8)里面的Qij
		Ghost_cells();//计算虚元
		Dissipative();//人工黏性
		...
		for (i = 1; i <= Imax - 1; i++)
		{
			for (j = 1; j <= Jmax - 1; j++)
			{
				...
				Cell[i][j].Med[1] = (Cell[i][j].QC[1] - Cell[i][j].D[1]) / Cell[i][j].Vol;
				Cell[i][j].U[1] = Cell[i][j].U_former[1] - RKalpha[z] * Cell[i][j].dt * Cell[i][j].Med[1];

				Cell[i][j].Med[2] = (Cell[i][j].QC[2] - Cell[i][j].D[2]) / Cell[i][j].Vol;
				Cell[i][j].U[2] = Cell[i][j].U_former[2] - RKalpha[z] * Cell[i][j].dt * Cell[i][j].Med[2];

				Cell[i][j].Med[3] = (Cell[i][j].QC[3] - Cell[i][j].D[3]) / Cell[i][j].Vol;
				Cell[i][j].U[3] = Cell[i][j].U_former[3] - RKalpha[z] * Cell[i][j].dt * Cell[i][j].Med[3];

				Cell[i][j].Med[4] = (Cell[i][j].QC[4] - Cell[i][j].D[4]) / Cell[i][j].Vol;
