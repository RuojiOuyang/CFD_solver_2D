...
void Convective()
{
	...
	for (i = 1; i <= Imax - 1; i++)
	{
		...
		FaceI[i][1].FU[5] = 0.5 * (Cell[i][1].U[5] + DummyIS[i][1].U[5]);
		...
		FaceI[i][Jmax].FU[5] = 0.5 * (Cell[i][Jmax - 1].U[5] + DummyIN[i][1].U[5]);
	}
	for (j = 1; j <= Jmax - 1; j++)
	{
		...
		FaceJ[1][j].FU[5] = 0.5 * (Cell[1][j].U[5] + DummyJW[1][j].U[5]);
		...
		FaceJ[Imax][j].FU[5] = 0.5 * (Cell[Imax - 1][j].U[5] + DummyJE[1][j].U[5]);
	}
	...
	for (j = 2; j <= Jmax - 1; j++)//i=1:Imax-1,j=2:Jmax-1对应的边都在内部
	{
		for (i = 1; i <= Imax - 1; i++)
		{
			...
			FaceI[i][j].FU[5] = 0.5 * (Cell[i][j - 1].U[5] + Cell[i][j].U[5]);
		}
	}
	for (j = 1; j <= Jmax - 1; j++)//i=2:Imax-1,j=1:Jmax-1对应的边都在内部
	{
		for (i = 2; i <= Imax - 1; i++)
		{
			...
			FaceJ[i][j].FU[5] = 0.5 * (Cell[i - 1][j].U[5] + Cell[i][j].U[5]);
		}
	}
	...
	for (j = 1; j <= Jmax; j++)
	{
		for (i = 1; i <= Imax - 1; i++)
		{
			...
			FaceI[i][j].FF[5] = FaceI[i][j].FU[5] * (FaceI[i][j].FU[2] / FaceI[i][j].FU[1] * FaceI[i][j].nx + FaceI[i][j].FU[3] / FaceI[i][j].FU[1] * FaceI[i][j].ny);
		}
	}
	for (j = 1; j <= Jmax - 1; j++)
	{
		for (i = 1; i <= Imax; i++)
		{
			...
			FaceJ[i][j].FF[5] = FaceJ[i][j].FU[5] * (FaceJ[i][j].FU[2] / FaceJ[i][j].FU[1] * FaceJ[i][j].nx + FaceJ[i][j].FU[3] / FaceJ[i][j].FU[1] * FaceJ[i][j].ny);
		}
	}
	...
	for (j = 1; j <= Jmax - 1; j++)
	{
		for (i = 1; i <= Imax - 1; i++)
		{
			...
			Cell[i][j].QC[5] = FaceI[i][j + 1].FF[5] - FaceI[i][j].FF[5] + FaceJ[i + 1][j].FF[5] - FaceJ[i][j].FF[5];
		}
	}
}