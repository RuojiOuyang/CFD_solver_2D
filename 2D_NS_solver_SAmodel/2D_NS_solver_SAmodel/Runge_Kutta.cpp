...
void Runge_Kutta()
{
	...
	for (i = 1; i <= Imax - 1; i++)
	{
		for (j = 1; j <= Jmax - 1; j++)
		{
			...
			Cell[i][j].U_former[5] = Cell[i][j].U[5];
		}
	}
	...
	for (z = 1; z <= 5; z++)
	{
		...
		for (i = 1; i <= Imax - 1; i++)
		{
			for (j = 1; j <= Jmax - 1; j++)
			{
				...
				Cell[i][j].Med[5] = (Cell[i][j].QC[5] - Cell[i][j].QV[5] - Cell[i][j].S - Cell[i][j].D[5]) / Cell[i][j].Vol;
				Cell[i][j].U[5] = Cell[i][j].U_former[5] - RKalpha[z] * Cell[i][j].dt * Cell[i][j].Med[5];
				if (Cell[i][j].U[5] < 0)
				{
					Cell[i][j].U[5] = Cell[i][j].U[1] * 1e-10;
				}
			}
		}
