...
void Ghost_cells()
{
	...
	for (i = 1; i <= Imax - 1; i++)
	{
		for (nd = 1; nd <= n_dummy; nd++)
		{
			...
			DummyIS[i][nd].Ma = sqrt((DummyIS[i][nd].u * DummyIS[i][nd].u + DummyIS[i][nd].v * DummyIS[i][nd].v) / DummyIS[i][nd].T);
			//cout << DummyIS[i][nd].Ma << endl;
		}
	}
	...
	for (i = 1; i <= Imax - 1; i++)
	{
		...
		DummyIN[i][1].Ma = sqrt((DummyIN[i][1].u * DummyIN[i][1].u + DummyIN[i][1].v * DummyIN[i][1].v) / DummyIN[i][1].T);
	...
	for (j = 1; j <= Jmax - 1; j++)//左边界
	{
		for (nd = 1; nd <= n_dummy; nd++)
		{
			...
			DummyJW[nd][j].Ma = sqrt((DummyJW[nd][j].u * DummyJW[nd][j].u + DummyJW[nd][j].v * DummyJW[nd][j].v) / DummyJW[nd][j].T);
		}
	}
	for (j = 1; j <= Jmax - 1; j++)//右边界
	{
		for (nd = 1; nd <= n_dummy; nd++)
		{
			...
			DummyJE[nd][j].Ma = sqrt((DummyJE[nd][j].u * DummyJE[nd][j].u + DummyJE[nd][j].v * DummyJE[nd][j].v) / DummyJE[nd][j].T);
		}
	}
